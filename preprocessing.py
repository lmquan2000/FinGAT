...
SP500_name = pd.read_csv("../datasets/sp500_companies.csv")
SP500_stock_df = pd.read_csv("../datasets/sp500_stocks.csv")
# SP500_name.head()
...
for target in SP500_name["Symbol"].unique():
    ...
    da["stock_price"] = SP500_stock_df[SP500_stock_df.Symbol == target].reset_index(drop=True)
...
for target in SP500_stock.keys():
    SP500_stock[target]["stock_price"] = SP500_stock[target]["stock_price"][SP500_stock[target]["stock_price"]["Date"].isin(need_day)].reset_index(
        drop=True
    )
...
def before_day(week):
    ...
    for w in range(week):
        train_x = []
        for tr_ind in range(len(train_data["AAPL"]) - 7 - (week - 2) - 1):
            tr = []
            for target in SP500_stock.keys():
    ...
    for w in range(week):
        test_x = []
        for te_ind in range(len(test_data["AAPL"]) - 7 - (week - 2) - 1):
            te = []
            for target in SP500_stock.keys():
...
data = before_day(4)
with open("../datasets/sp500_data.pkl", "wb") as f:
    pickle.dump(data, f)
f.close()
